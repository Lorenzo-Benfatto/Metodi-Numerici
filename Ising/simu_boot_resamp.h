...
void simulazione(float beta, FILE *misure, int i_decorrel){
    
    FILE *f  ; //*l;
    int x; //variabili per leggere il file
    int iflag, measures; //nomi dei parametri della simulazione
    ...
    x = fscanf(f, "%d  %d  %f", &iflag, &measures, &extfield);
    ...
    for(int iter=0; iter<measures; iter++){
        ...
        fprintf(misure, "%f  %f  %d %f %d \n", magnetization(xmagn), energy(xene), iter, beta, i_decorrel);
