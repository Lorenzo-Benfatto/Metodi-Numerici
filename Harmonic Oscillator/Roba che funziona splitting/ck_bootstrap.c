...
#include <unistd.h>
#include <errno.h>
#include"/Users/margherita/Desktop/UNIPI/QUINT_ANNO/Ising/usefulstuff.h"
#include"/Users/margherita/Desktop/UNIPI/QUINT_ANNO/Ising/ran2.h"
#include"/Users/margherita/Desktop/UNIPI/QUINT_ANNO/Ising/listfunction.h"
...
void bootstrap(FILE *f, float e, int enne,int tau, char string[10], int altro, int N_misure){
    ...
    //if(e==z){
        ...
        if (altro==1){
            ...
            sprintf(filemisure,"/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Bootstrap/resample_binned_%s(eta=%.3f,N=%d).txt", string,e, enne);
            
            
        }
        else {sprintf(filemisure,"/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Bootstrap/resample_binned_ck_%s(eta=%.3f,N=%d,k=%d).txt", string,e, enne, tau);
...
void read_file_input(){ /*Mi serve solo per leggere il numero di misure N e la larghezza del reticolo L,
 gli altri non mi servono a niente, quindi non gli do nemmeno un nome significativo*/
    FILE *in;
    in = fopen("/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/input_split.txt", "r");
    control_file(in);
    int x, iflag, i_term; 
    float eta, d_metro;
...
int main(){
    ...
    f=fopen("/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/valori_split.txt","r");  //leggo il file valori.txt
    control_file(f);
    ...
    while(Nlatt!=NULL){
        ...
        for(int k=0; k<max_k; k++){
            ...
            //prima volta
            char string[10]="";
            sprintf(filemisure, "/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Campo/C(k)/Ck_Y%s(eta=%.3f,N_Eta=%.0f,k=%d).txt", string, eta, ennepereta,k); //it modifies each time the name of the file to etae created
            ...
            //seconda volta
            char string2[10]="2";
            sprintf(filemisure, "/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Campo/C(k)/Ck_Y%s(eta=%.3f,N_Eta=%.0f,k=%d).txt", string2, eta, ennepereta,k); //it modifies each time the name of the file to etae created
            misure[p]=fopen(filemisure, "r");
            printf("file: %s \n", filemisure);
            control_file(misure[p]);
            N_misure=n*idec;
            ...
            //prima volta <Y>
            char string3[10]="<Y>";
            sprintf(filemisure, "/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Campo/C(k)/%s(eta=%.3f,N_Eta=%.0f).txt",  string3, eta, ennepereta); //it modifies each time the name of the file to etae created
            misure[p]=fopen(filemisure, "r");
            printf("file: %s \n", filemisure);
            control_file(misure[p]);
            altro=1;

            bootstrap(misure[p], eta, ennelatt, k, string3, altro, ennelatt);

            //prima volta <Y2>
            char string4[10]="<Y2>";
            sprintf(filemisure, "/Users/margherita/Desktop/UNIPI/QUINT_ANNO/OscillatoreArmonico/Splitting/Campo/C(k)/%s(eta=%.3f,N_Eta=%.0f).txt", string4, eta, ennepereta); //it modifies each time the name of the file to etae created
            misure[p]=fopen(filemisure, "r");
            printf("file: %s \n", filemisure);
            control_file(misure[p]);
            altro=1;

            bootstrap(misure[p], eta, ennelatt, k, string4, altro, ennelatt);
