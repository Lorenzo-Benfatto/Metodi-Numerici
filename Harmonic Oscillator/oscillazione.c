...
void update_metropolis(){
    ...
    for(int i = 1; i<Nlatt-1; i++){
        ...
        acc=fopen("/mnt/c/Users/aministratore/Documents/Università/Magistrale/Metodi Numerici/Modulo-3/Nuova_run/Oscillazione.txt","a");
        control_file(acc);
        if(x<p_rat){
...
double * measure(double obs[3]){
    ...
    obs[2] = obs[2]/Nlatt;  //media dei campi
...
int main(){
    ...
    control_file(lat);
    printf("2\n");
    misure = fopen("/mnt/c/Users/aministratore/Documents/Università/Magistrale/Metodi Numerici/Modulo-3/Nuova_run/Oscillo/misure.txt","w" );
    printf("3\n");
    control_file(misure);
    printf("4\n");
    x= fscanf(input, "%f  %f  %d  %d  %d  %d  %d", &eta, &d_metro, &measures, &i_decorrel, &iflag, &i_term, &l);
