...
//definisco il tipo lista che contiene elemento numerico int più puntatore a blocchetto successivo
...
list_int *agg_testa_int(int n, list_int *p){ // argomenti sono il valore dell'elemento da aggiungere e il puntatore alla testa lista
    if(p != NULL){
        list_int *tmp = p; //salvo in tmp l'indirizzo del secondo elem della lista
        p=(list_int*)malloc(sizeof(list_int)); // alloco nuovo spazio da aggiungere alla lista, costituirà il nuovo primo elem
        p->val=n;                         //assegno primp elem
        p->next=tmp;                //così lo colloco in testa, dato che punta al secondo elemento
    }
    else{ //se la lista è vuota aggiungo semplicemente un elemento
...
/* */
list_int *agg_coda_int(int n, list_int *p){ // argomenti sono il valore dell'elemento da aggiungere e il puntatore alla testa lista
    if(p != NULL){
        list_int *tmp = p;
        while(tmp->next!=NULL){ //questo ciclo mi serve per arrivare in fondo alla lista
            tmp=tmp->next;
        }
        tmp->next=agg_testa_int(n, NULL);
    }
    else{ // se la lista è vuota aggiungo semplicemente un elemento
        p=agg_testa_int(n, p);
    }
...
//cerca un elemento specifico, utile ad esempio per le rm_posizione o agg_posizione
// Non uso un puntatore ausiliario per scorrere la lista, tanto ritorna un intero
int cerca_int(int n,list* testa){
	int posizione = -1;
	int trovato = 0;
	int i=0;
	while(testa != NULL && trovato == 0){
...
list *move_to_position(int position, list * testa){
...
list_int *scan_file_interi(FILE *f,list_int *d){
    int elem;
    int x;
    x=fscanf(f, "%d", &elem);
    while (x!=EOF){
            d=agg_coda_int(elem, d);
            x=fscanf(f, "%d ", &elem);
        }
    //print_lista(d);
    return d;
...
/*      NON SERVE PIÙ
 ...
 Come sopra, ma legge a partire dall'ultimo elemento del file (ultima riga)

list *scan_file_testa(FILE *f,list *d){
    float elem;
    int x;
    x=fscanf(f, "%f", &elem);
    while (x!=EOF){
