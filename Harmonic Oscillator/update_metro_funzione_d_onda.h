...
void update_metropolis_saving_field(float eta){
    float c1, c2; //sono solo shortcut per scrivere 1/eta e altra funz di eta
    int ip, im; //coordinate dei 2 primi vicini
    float force, phi, phi_prova;  //force = forza del campo intorno; phi = valore attuale del campo; phi_prova = valore di prova del campo
    float p_rat, x; // p_rat = valore di energia con cui confrontare x che è estratto a caso e mi dirà se accettare il passo o no
    ...
    campo=fopen("/mnt/c/Users/aministratore/Documents/Università/Magistrale/Metodi Numerici/Modulo-3/Nuova_run/FunzioneDondaFondamentale/Valoriperisto.txt","a");
    control_file(campo);
...
double * measure(double obs[3]){
    ...
    obs[0] = obs[0]/(1.0*Nlatt);  //media sul singolo path di y**2 sommato su tutti i path
    obs[1] = obs[1]/(1.0*Nlatt);  //media sul singolo path di Delta y**2 sommato su tutti i path
    obs[2] = obs[2]/(1.0*Nlatt);  //media sul singolo path di field
   ...
   //     else if(eta<0.09 && eta>=0.05) d_metro=0.5;
    ...
    for (int iter=0; iter<measures; iter++){
        ...
        for(int idec=0; idec<i_decorrel; idec++ ){
            ...
            if(idec!=25) update_metropolis(eta);
            else update_metropolis_saving_field(eta);
